...
# Methylation (beta values adjusted for cell type) ~ 1 + age + sex + PRS + 3 Genomic PC's + Smoking Score + Medication History + Missing PRS genotypes
...
temp.design <- model.matrix(~sample.annot$PRS+temp.sex+sample.annot$Age+sample.annot$PC1+sample.annot$PC2+sample.annot$PC3+sample.annot$smoking.score+sample.annot$Medication+sample.annot$Missing.genos)
