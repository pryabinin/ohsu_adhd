...
# Methylation (beta values adjusted for cell type) ~ 1 + age + sex + status + 3 Genomic PC's + Smoking Score + Medication History
...
temp.design <- model.matrix(~temp.status+temp.sex+sample.annot$Age+sample.annot$PC1+sample.annot$PC2+sample.annot$PC3+sample.annot$smoking.score+sample.annot$Medication)
results.fit <- lmFit(adj.betas, temp.design)
results.fit <- eBayes(results.fit)

# Save the results object
save(results.fit,file="main_model_fit_object.Rdata")
